---
title: Dashboard
description: Guide to using the HyOS web dashboard for server management
---
...
| `HYTALE_SERVER_PORT` | `30381` | REST API port on the server |
...
| `SESSION_SECRET` | — | Encryption key for session cookies (32+ chars). Auto-generated if not set. |
| `COOKIE_SECURE` | `false` | Set to `true` to require HTTPS for session cookies. |
...
See [Mods & Plugins](/docs/mods) for details on content-only mod patching.
