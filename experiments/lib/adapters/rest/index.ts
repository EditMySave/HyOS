...
export class RestAdapter implements ServerAdapter {
  ...
  async revokePermission(
    ...
    return this.apiRequest<PermissionsInfo>(
  ...
  async getWorlds(): Promise<WorldInfo[]> {
    const data = await this.apiRequest<{ count: number; worlds: WorldInfo[] }>(
      "/worlds",
    );
    return data.worlds;
  }
  ...
  async getBlock(
    ...
    return this.apiRequest<BlockInfo>(
      `/worlds/${worldId}/blocks/${x}/${y}/${z}`,
    );
  }
